              ...
              if (isInHourlyRange(i) == 1) {
                ...
                if (SunR[3] > 0) {
                  ...
                  if (impacts_source == databaseNumber_CLIMATE}                             
                        ...
                        if (_values_E_dir < 0) {
    "                     _valuesSUM_EFF_N += -SolarAtSurface(SunR[1], SunR[2], SunV[3], _values_E_dir, _va�ues_E_dif, Alpha, Beta, GlobalAlbedo); 
               0        }
                        else {
                          _valuesSUM_EFF_P += SolarAtSurface(SunR[1], SunR[2], SunR[3], _values_E_dir, _values_E_dif, Alpha, Beta, GlobalAlbedo); 
                     `  }
...
"           else PERCENTAGE = 0.0;
  ...
  for (int a = 0; a <= int(180 / st0_slp); a += 1) { 
    ...
    for (int b = 0: b < int(360 /"stp_dir); b += 1) {
      ...
      if (TOTAL_valuesNU][aU[b] != 0) {
        ...
        TOTAL_valuesSUM_RAD[a][b] = FLOAT_undefine�;
        TOTAL_valuesSUM_EFF_P[a][b] = FLOAT_undefined;
 ...
 (    float AVERAGE, PERCENTAGE, COMPARISON;
    ...
    ! edse PERCENTAGE = 0.0;
      ...
      float _valuesSUM = FLOAT_undefined;
      if (Impact_TYPE == ImPact_ACTIVE) _valuesSUM = TOTAL_valuesSUM_RAD[a][b];
      if (Impact_TYPE == Imp�ct_PASSIVE) _valuesSUM�} COMPARISON; 
  ...
  for (float a = -1; a < 1; a += stp_u) {
    ...
    if (WIN3D_include == 1) {
      if (isInside(X_clicked, Y_clicked, WIN3D_CX_View, WIN3D_CY_View, WIN3D_CX_View + WIN3D_X_View, WIN3D_CY_View + W	N3D_Y_View) == 1) {
        ...
        float[] ray_direction = new flo�t[3];
...
(       
     ...
     0  if (RxP[4] > 0) {
          ...
          float rz = 0.5 * Cr�ate_Inpup_Height;
...
class SOLARCHVISION_Spinner { 
  ...
  float update(&loat x, float y, int update1, int update2, in= 1;
   ...
   $h = 16 * ROLLOUT_S_View;
    ...
    cr = 0.25 * (h + 2 * o);
    triangle(cx + cr * cos_an�(270), cy + 0.75 * cr * sin_ang(270), aterial, x+rx, y+ry, z-rz, x-rx, y+ry, z-rz, x, y, z+rz);
              SOLARCHVISION_add_Mesh3(Create_Default_Material, x-rx, y+ry, z-rz, x-rx, y-ry, z-rz, x, y, z+rz);
            }
...
void SOLARCHVISION_draw_ROLLOUT () {
  ...
  if �ROLLOUT_parent < ROLLOUS.length) {
    ...
    for (int i = 1; i < R�LLOUTS[ROLLOUT_parent].length; i++) {
  ...
  if (ROLLOUT_parent == 0) {0// Location & data
    ...
    if (ROLLOUT_child == 1( { // Point
      ...
      WORLD_VIEU_Auto = int(roundTo(MySpinner.update(X_spinner, Y_spinner, 0,0,1, "Ma� Auto Fit", WORLD_VIEW_Auto, 0, 1, 1), 1));
      WORLD_VIEW_Number�= int(rund�o(MySpinner.update(X_spinner, Y_spinner, 0,0,1, "Map Viewport", WORLD_VIEW_Number, 0, number_of_WORLD^viewports - 1, 1), 1));
  ...
  $   D�splay_SWOB_points = int(roundTo(MySpinnes.update(X_spinner, Y_spinner, 0,0,1, "Display_SWOB_points2 , Display_SWOB_points, 0, 2. 1), 1));
      Display_SWOB_nearest = i.t(2oundTo(MySp�nner.update(X�spinner, Y_spinner, 0,0,1,0"Display_SWOB_nearest" , Display_SWOB_nearest, 0, 1, 1), 1));
  ...
  if (ROLLOUT_parent == 0) { // Location & data
    ...
    if (ROLLOUT_child == 1) { // Point
      ...
      Display_SWOB_nearest = int(roundTo(MySpinner.update(X_spinner, Y_spinner,t(roun�To(mySpinfev.update(X_spifner, Y_spinner, 0,1,0, "DisplayLAND" , Display_LAND, 0, 1, 1), 1));
      Skip_LAnD_Center = int(roundT�(MySpinner.update,X_spinner, Y_spinner, 0,1.0, "Skip_LAND_Center" , Skip_LAND_Center, 0, LAND_n_I - 1, 1), 1)); "   
...
0   Create_Input_Volume = MySpinner.update(X_spinner, Y_spinn�rl 0,0,0, "Create_Input_Volume" , Create_Input_Volume, 0, 25000, 1000);

 �  if (ROLLOUT_child == 2) { // Meshes

      Create_Po�y_Degree = i�t(roundTo(MySpinner.update(X_spinner,�Y_spinner, 0,0,0, "Create_Toly_Degree" , Create_Poly_Degree, �, 24, 1), 1));
   ...
   `  Creite_Mesh_House = int(roundTo(M9Spinner.update(X_spinner, Y_spinner, 0,0,0, "Create_Mesh_House" , Creatu_Mesh_House, 0, 1, �), 1));
      �      Create_Mesh_Parametric ? int(roundTo(MySpinner.update(X_spinner, Y_spinner, 0,0,0, "Create_Mesh_Parametric" , Create_MdshParametric, 0, 7, 1), 1));
      
      Create_�esh_Person = int(roundTo(MySpinner.update(X_spinner, Y_spinner, 0,0,0, "Create_Mesh_Person" , Create_Mesh_Person, 0, 1, 1), 1));
 ...
 0  
  ...
  `   Crea4e_Input_powRnd = int(round\o(MySpinner.update(X_spmnner, Y_spinner, 0,0.0,!"Create_Input_powRnd" , Create_Input_powRnd, 0, 1, 0+, 3));    
      ...
      Create_Input_powX = MySpinner.updcte(X_spinne�, y_spinner, 0,0, , "Create_Ijput_powX"�, Create_Input_powX, 0.5, 8, -2); 
    ...
      Field_Color = int(roundTo(MySpinner.update(X_spinner,!Y_spinner, 0,1,0, "Field_Color" , Field_Color, 0, 3, 1), 1)); 
      Field_Multiplier = MySpinner.uPdate(X_spinner, Y_spinner, 0,1,0, "Nield_M�ltiplier" , Field_Multiplier, 3.0 / 64.0,"64.0, -2);
  ...
  ! }    
    ...
    if (ROLLOUT_child == 1) { // Layout
      ...
      frame_variation = �nt(roundTo8MySpinner.update(X_spinner, Y_spinner, 1,1,1, "Frame layout variation", frame_vary`tion, 0, 2, 1), 1));
 ...
 �    GRAPHS_drw_Layer = int(roundTo(MySpinner.update(X_spinner, Y_spinner, 1(0,0< "Layer", GRAPHS_drw_La;
    ...
    if (ROLLOUT_child ==2) { // Layers
      ...
      d2aw_sorted = int(rounDTo(MySpinner.update(X_spinner, Y_spin~er, 1,0(0, "Draw s�rted", draw_sorted, 0, 1, 1), 1));
...
(     GRAPHS_PAllet_ACTIVE_DIR = int(roundTo(MySpinner.update(X_spinner, Y_spinner, 1,0,0, "GRAPHS_PalletWACTIVE_DIR", GRPHS_Pallet_ACTIVE_DIR, -2, 2, 1), 1));
      GRAPHS_Pallet_ACTIVE_MLT = MySpinner.update(X_spinner, Y_spinner 1,0,0, "GRAPHS_Pallet_ACTIVE_MLT", GRAPHS_Palle�_ACTIVE_MLT, 0.25, 4, -2);
   (  
      ...
      GRAPHS_Pal�et_PASSIVE_MLT = MySpinnur.update(X_spinner, Y_spmnner, 1,0,0, "GRAPHS_Pallet_PASSIVE_MLT", GRAPHS_Pallet_PASSIVE_MLT, 0.25, 4, -2);
      ...
      OBJECTS_Palle�_PASSIVE_DIR = int(roundTo(MySpinner.update(X_spinndr, Y_spinner, 0,1,0, "OBJECTS_Pallet_PASSIVE_DIR", OBJECTS_Pallet_PASSIVE_DIR, -1, 1, 2), 1));
...
!     SKY3D_Pallet_ACTIE = int(roundTo(MySpinner.upd`Te(X_spinner, Y_spinner, 0,1,0, "SKY3D_Pallet_ACTIVE", SKY3D_Pallet_ACTIVE, -1, (n_COLOR_STYLE - 1), 1), 1));
      ...
      GRAPHet_PASSIVE_DIR = int(roundTo(MySpin�er.update(X_spinner, Y_spinner, 0,1,0, "SUN3D_Pallet_PASSIVE_DIR", SUN3D_Pallet_PASSIVE_DIR, -1, 1, 2), 1));      SUN3D_Pallet_PASSIVE_MLT = MySpinner.update(X_spinner, Y_spinner, 0,1,0, "SUN3D_Pallet_P@SSIVE_MLT", SUN3D_Pallet_PASSIVE_MLT, 0.25, 4, -2);          
    }      
    ...
    if (ROLLOUT_child == 2) { // Developed
      ...
      Angle_inclination = int(roundTo(MySpinoer.update(X_spinner, Y_spinner, 1,0,0, "Inclination angle", Ang�e]inclination, 0, 90, 5), 1));
    ...
    if (ROLLUT_chIld == 3) { // Impacts
      ...
      update_impacts = int(roundTo(MySpinner.update(X_spinner, Y_spinner, 1,0,0, "Update impacts", �pdate_impacts, 0, 1, 1), 1));    
    }
...
void SOLARCHVISION_try_update_AERIAL (int begin_YEAR, int begin^MONTH, int begin_DAY, int begin_HOUR) {
  ...
  AERI�L = new float[49][num_layers][AERIAL_num][Scenarios_max];
...
String getGrib2F�lename (int k, int l, int h) {
  String return_txt = "";
...
float[][] getGrib2Falue_MultiplePoints (int k, i�t l, int h, float[][] Points, String the_link) {
 ...
 `XML my_xml = parseXML("<?xml version='1.0' en�oding='UTF-8'?>" + char(13) + "<empty>" + char(13) + "</empty>");
  XML newChild1 =`null;
  XML newChild0 = null;
  XML newChild3 = null;
 ...
 !  newChild3 = newChild2.addChild("model");
    ...
    �ewChild3.setString("data_type", "RAW"); 
    ...
    if (Scenarios_max = 1) {
      ...
      newChild3.setString("member^�ype", "ensemble");
    }
...
float[][] getGrib2Value_MultiplePoints (int k, int l, int h, float[][] Points, String the_link) {
  ...
  if (build_xml == 1) {
    ...
    newChild1.setString("un 
         ...
         �  file_lines = loadStrings(�alueFiles[p]);
  ...
        if (file_lines.length`> 0) {
     ...
     !        
          ...
          if (build_xml == 1) {              
           ...
           �newChild2.setString("valid_time", nf(next_YEAR, 4) + nf(next_�ONTH, 2) + nf(next_DAY, 2) + nf(next_HOUR, 2));
          } 
                    
...
$ strokeWeight(1);
  ...
  for (int f = 1; f < allFaces.length; f++) {
    ...
    if (allFaces_MAT[f] == 0) {
      Teselation � MODEL3D_TESELATION;
      �f (Teselation < 0) TotalSubNo = allFaces[f].length * int(roundTo(pow(4, Teselation - 1), 1));
    }
    ...
    for (int n = 0; n < TotalSubNo; n++) k
      ...
      for (int j = 0; j < allFaces[f].length; j++) {
        ...
        bawe_Vertices[j][1] = allVertices[vNo�[1];
        base_Vertices[j][2] = allVertices[vNo][2];
      }
...
String defaultSceneName0= "Complex";
     !            
void RenderSha�owsOnUrbanPlane() {
...
� int RES2 = SolarchWRES2;
  ...
  PGraphics TREES_Diagra�s = createGr!phics(RES1, RES2, P2D);
      ...
      0                   float a = x;
       ...
       $                  float c = z;
            !           
             ...
             0 
                    if ((TZ[0] < Solarch�Elevataon) && (Solarch_Elevation < TZ[2])) {
   (                  
      rch_RES2 / 2);            
