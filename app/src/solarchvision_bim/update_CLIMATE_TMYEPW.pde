void update_CLIMATE_TMYEPW () {
  ...
  CLIMATE_TMYEPW_values = new float [24][365][allLayers.length][(1 + CLIMATE_TMYEPW_end - CLIMATE_TMYEPW_start)];
  CLIMATE_TMYEPW_flags = new boolean [24][365][allLayers.length][(1 + CLIMATE_TMYEPW_end - CLIMATE_TMYEPW_start)]; // true: direct input , false: no-input, interpolated or post-processed
