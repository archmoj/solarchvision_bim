  ...
  void drawView () {
    ...
    if (this.parent == PARENT_PERIOD_SCENARIOS) {
      ...
      if (this.child == CHILD_PERIOD_TIME) {
        ...
        TIME.date = this.Spinner(X_control, Y_control, 1, 1, 0, "Days past March equinox", TIME.date, 0, 364, 1, 1);

        //TIME.beginDay = this.Spinner(X_control, Y_control, 1, 1, 0, "Day of year (0-364)", TIME.beginDay, 0, 364, 1, 1);

        TIME.day = this.Spinner(X_control, Y_control, 1, 1, 0, "Start day", TIME.day, 1, 31, 1, 1);
        TIME.month = this.Spinner(X_control, Y_control, 1, 1, 0, "Start month", TIME.month, 1, 12, 1, 1);
        TIME.year = this.Spinner(X_control, Y_control, 1, 1, 0, "Start year", TIME.year, 1953, 2100, 1, 1);
      }

      if (this.child == CHILD_PERIOD_RANGES) {
        STUDY.i_Start = this.Spinner(X_control, Y_control, 1, 0, 0, "Start hour", STUDY.i_Start, 0, 23, 1, 1);
        STUDY.i_End = this.Spinner(X_control, Y_control, 1, 0, 0, "End hour", STUDY.i_End, 0, 23, 1, 1);

        SampleYear_Start = this.Spinner(X_control, Y_control, 1, 0, 0, "Start year", SampleYear_Start, CLIMATE_CWEEDS_start, CLIMATE_CLMREC_end, 1, 1);
        SampleYear_End = this.Spinner(X_control, Y_control, 1, 0, 0, "End year", SampleYear_End, CLIMATE_CWEEDS_start, CLIMATE_CLMREC_end, 1, 1);

        SampleMember_Start = this.Spinner(X_control, Y_control, 1, 0, 0, "Start member", SampleMember_Start, ENSEMBLE_FORECAST_start, ENSEMBLE_FORECAST_end, 1, 1);
        SampleMember_End = this.Spinner(X_control, Y_control, 1, 0, 0, "End member", SampleMember_End, ENSEMBLE_FORECAST_start, ENSEMBLE_FORECAST_end, 1, 1);

        SampleStation_Start = this.Spinner(X_control, Y_control, 1, 0, 0, "Start station", SampleStation_Start, ENSEMBLE_OBSERVED_start, ENSEMBLE_OBSERVED_end, 1, 1);
        SampleStation_End = this.Spinner(X_control, Y_control, 1, 0, 0, "End station", SampleStation_End, ENSEMBLE_OBSERVED_start, ENSEMBLE_OBSERVED_end, 1, 1);

        ENSEMBLE_OBSERVED_maxDays = this.Spinner(X_control, Y_control, 0, 0, 1, "Forecast/Obs_maxDays", ENSEMBLE_OBSERVED_maxDays, 0, 31, 1, 1);
      }

      if (this.child == CHILD_PERIOD_FILTERS) {

        STUDY.skyScenario = this.Spinner(X_control, Y_control, 1, 0, 0, "Sky status", STUDY.skyScenario, 1, 4, 1, 1);
        STUDY.filter = this.Spinner(X_control, Y_control, 1, 0, 0, "Hourly/daily filter", STUDY.filter, 0, 1, 1, 1);
      }
      ...
      if (this.child == CHILD_LOCATION_POINT) {
        ...
        //WORLD.autoView = this.Spinner(X_control, Y_control, 0,0,1, "Map Auto Fit", WORLD.autoView, 0, 1, 1, 1);
        //WORLD.VIEW_id = this.Spinner(X_control, Y_control, 0,0,1, "Map Viewport", WORLD.VIEW_id, 0, WORLD.numMaps - 1, 1, 1);
      ...
      if (this.child == CHILD_LOCATION_STATIONS) {
        ...
        WORLD.displayAll_TMYEPW = this.Spinner(X_control, Y_control, 0, 0, 1, "displayAll_TMYEPW", WORLD.displayAll_TMYEPW, 0, 2, 1, 1);
        ...
        WORLD.displayAll_CWEEDS = this.Spinner(X_control, Y_control, 0, 0, 1, "displayAll_CWEEDS", WORLD.displayAll_CWEEDS, 0, 2, 1, 1);
        ...
        WORLD.displayAll_CLMREC = this.Spinner(X_control, Y_control, 0, 0, 1, "displayAll_CLMREC", WORLD.displayAll_CLMREC, 0, 2, 1, 1);
        ...
        WORLD.displayAll_SWOB = this.Spinner(X_control, Y_control, 0, 0, 1, "displayAll_SWOB", WORLD.displayAll_SWOB, 0, 2, 1, 1);
        ...
        WORLD.displayAll_NAEFS = this.Spinner(X_control, Y_control, 0, 0, 1, "displayAll_NAEFS", WORLD.displayAll_NAEFS, 0, 2, 1, 1);
      ...
      if (this.child == CHILD_GEOMETRY_CREATE) {
        ...
        User3D.default_Material = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-create.Material", User3D.default_Material, -1, 8, 1, 1);
        User3D.default_Tessellation = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-create.Tessellation", User3D.default_Tessellation, 0, 6, 1, 1);
        User3D.default_Layer = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-create.Layer", User3D.default_Layer, 0, 16, 1, 1);
        User3D.default_Visibility = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-create.Visibility", User3D.default_Visibility, -1, 1, 1, 1);
        User3D.default_Weight = this.Spinner(X_control, Y_control, 0,0,0, "3D-create.Weight" , User3D.default_Weight, -20, 20, 1, 1);
        User3D.default_Closed = this.Spinner(X_control, Y_control, 0,0,0, "3D-create.Closed" , User3D.default_Closed, 0, 1, 1, 1);
        ...
        User3D.create_Snap = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-create.Snap", User3D.create_Snap, 0, 1, 1, 1);

        User3D.create_SphereDegree = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-create.SphereDegree", User3D.create_SphereDegree, 0, 5, 1, 1);

        User3D.create_CylinderDegree = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-create.CylinderDegree", User3D.create_CylinderDegree, 3, 36, 1, 1);

        User3D.create_PolyDegree = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-create.PolyDegree", User3D.create_PolyDegree, 3, 36, 1, 1);

        User3D.create_Parametric_Type = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-create.Parametric_Type", User3D.create_Parametric_Type, 1, 6, 1, 1);
        User3D.create_Person_Type = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-create.Person_Type", User3D.create_Person_Type, 0, allModel2Ds.num_files_PEOPLE, 1, 1);
        User3D.create_Plant_Type = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-create.Plant_Type", User3D.create_Plant_Type, 0, allModel2Ds.num_files_TREES, 1, 1);
      ...
      if (this.child == CHILD_GEOMETRY_MODIFY) {
        ...
        User3D.modify_TessellateRows = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-modify.TessellateRows", User3D.modify_TessellateRows, 1, 100, 1, 1);
        User3D.modify_TessellateColumns = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-modify.TessellateColumns", User3D.modify_TessellateColumns, 1, 100, 1, 1);
        ...
        Select3D.alignZ = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-select.alignZ", Select3D.alignZ, -1, 1, 1, 1);
      }

      if (this.child == CHILD_GEOMETRY_SOLID) {

        //User3D.create_powRnd = this.Spinner(X_control, Y_control, 0,0,0, "powRnd" , User3D.create_powRnd, 0, 1, 1, 1);
      ...
      if (this.child == CHILD_GEOMETRY_ENVIRONMENT) {
        ...
        Land3D.skipStart = this.Spinner(X_control, Y_control, 0, 1, 0, "Land3D.skipStart", Land3D.skipStart, 0, Land3D.num_rows - 1, 1, 1);
        Land3D.skipEnd = this.Spinner(X_control, Y_control, 0, 1, 0, "Land3D.skipEnd", Land3D.skipEnd, 0, Land3D.num_rows - 1, 1, 1);
      ...
      if (this.child == CHILD_GEOMETRY_VIEWPORT) {
        ...
        WIN3D.currentCamera = this.Spinner(X_control, Y_control, 0, 1, 0, "currentCamera", WIN3D.currentCamera, 0, allCameras.num, 1, 1);
        ...
        //WIN3D.FacesShade = this.Spinner(X_control, Y_control, 0,1,0, "FacesShade", WIN3D.FacesShade, 0, SHADE.Options_num - 1, 1, 1);
      ...
      if (this.child == CHILD_GEOMETRY_SIMULATION) {
        ...
        IMPACTS_displayDay = this.Spinner(X_control, Y_control, 0, 1, 0, "IMPACTS_displayDay", IMPACTS_displayDay, 0, STUDY.j_End - STUDY.j_Start, 1, 1);
        ...
        allSolarImpacts.sectionType = this.Spinner(X_control, Y_control, 0, 1, 0, "allSolarImpacts.sectionType", allSolarImpacts.sectionType, 0, 3, 1, 1);
        allSolidImpacts.sectionType = this.Spinner(X_control, Y_control, 0, 1, 0, "allSolidImpacts.sectionType", allSolidImpacts.sectionType, 0, 3, 1, 1);
        ...
        allSolidImpacts.Process_subDivisions = this.Spinner(X_control, Y_control, 0, 0, 0, "allSolidImpacts.Process_subDivisions", allSolidImpacts.Process_subDivisions, 0, 3, 1, 1);
      ...
      if (this.child == CHILD_GEOMETRY_OTHER) {
        ...
        allFaces.displayTessellation = this.Spinner(X_control, Y_control, 0, 1, 0, "3D-create.displayTessellation", allFaces.displayTessellation, 0, 4, 1, 1);

        Land3D.displayTessellation = this.Spinner(X_control, Y_control, 0, 1, 0, "Land.displayTessellation", Land3D.displayTessellation, 0, 4, 1, 1);
      ...
      if (this.child == CHILD_ILLUSTRATION_2D_LAYERS) {
        STUDY.plotSetup = this.Spinner(X_control, Y_control, 1, 0, 0, "Diagram setup", STUDY.plotSetup, -2, 8, 1, 1);
        ...
        STUDY.sumInterval = this.Spinner(X_control, Y_control, 1, 0, 0, "Probabilities interval", STUDY.sumInterval, 1, 24, 1, 1);
      ...
      if (this.child == CHILD_ILLUSTRATION_2D_COLORS) {
        ...
        //COLOR_STYLE_Current = this.Spinner(X_control, Y_control, 1,0,0, "Hourly color scheme", COLOR_STYLE_Current, -1, (COLOR_STYLE_Number - 1), 1, 1);

        STUDY.ACTIVE_palette_CLR = this.Spinner(X_control, Y_control, 1, 0, 0, "STUDY.ACTIVE_palette_CLR", STUDY.ACTIVE_palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
        STUDY.ACTIVE_palette_DIR = this.Spinner(X_control, Y_control, 1, 0, 0, "STUDY.ACTIVE_palette_DIR", STUDY.ACTIVE_palette_DIR, -2, 2, 1, 1);
        ...
        STUDY.PASSIVE_palette_CLR = this.Spinner(X_control, Y_control, 1, 0, 0, "STUDY.PASSIVE_palette_CLR", STUDY.PASSIVE_palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
        ...
        STUDY.SORT_palette_CLR = this.Spinner(X_control, Y_control, 1, 0, 0, "STUDY.SORT_palette_CLR", STUDY.SORT_palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
        ...
        STUDY.PROB_palette_CLR = this.Spinner(X_control, Y_control, 1, 0, 0, "STUDY.PROB_palette_CLR", STUDY.PROB_palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
      ...
      if (this.child == CHILD_ILLUSTRATION_3D_SOLAR) {
        ...
        allFaces.ACTIVE_palette_CLR = this.Spinner(X_control, Y_control, 0, 1, 0, "allFaces.ACTIVE_palette_CLR", allFaces.ACTIVE_palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
        allFaces.ACTIVE_palette_DIR = this.Spinner(X_control, Y_control, 0, 1, 0, "allFaces.ACTIVE_palette_DIR", allFaces.ACTIVE_palette_DIR, -2, 2, 1, 1);
        ...
        allFaces.PASSIVE_palette_CLR = this.Spinner(X_control, Y_control, 0, 1, 0, "allFaces.PASSIVE_palette_CLR", allFaces.PASSIVE_palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
        ...
        Sky3D.ACTIVE_palette_CLR = this.Spinner(X_control, Y_control, 0, 1, 0, "Sky3D.ACTIVE_palette_CLR", Sky3D.ACTIVE_palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
        Sky3D.ACTIVE_palette_DIR = this.Spinner(X_control, Y_control, 0, 1, 0, "Sky3D.ACTIVE_palette_DIR", Sky3D.ACTIVE_palette_DIR, -2, 2, 1, 1);
        ...
        Sky3D.PASSIVE_palette_CLR = this.Spinner(X_control, Y_control, 0, 1, 0, "Sky3D.PASSIVE_palette_CLR", Sky3D.PASSIVE_palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
        ...
        Sun3D.ACTIVE_palette_CLR = this.Spinner(X_control, Y_control, 0, 1, 0, "Sun3D.ACTIVE_palette_CLR", Sun3D.ACTIVE_palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
        Sun3D.ACTIVE_palette_DIR = this.Spinner(X_control, Y_control, 0, 1, 0, "Sun3D.ACTIVE_palette_DIR", Sun3D.ACTIVE_palette_DIR, -2, 2, 1, 1);
        ...
        Sun3D.PASSIVE_palette_CLR = this.Spinner(X_control, Y_control, 0, 1, 0, "Sun3D.PASSIVE_palette_CLR", Sun3D.PASSIVE_palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
      ...
      if (this.child == CHILD_ILLUSTRATION_3D_SPATIAL) {
        ...
        allSolids.palette_CLR = this.Spinner(X_control, Y_control, 0, 1, 0, "allSolids.palette_CLR", allSolids.palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
        ...
        Land3D.palette_CLR = this.Spinner(X_control, Y_control, 0, 1, 0, "Land3D.palette_CLR", Land3D.palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
        ...
        allWindFlows.palette_CLR = this.Spinner(X_control, Y_control, 0, 1, 0, "allWindFlows.palette_CLR", allWindFlows.palette_CLR, -1, (COLOR_STYLE_Number - 1), 1, 1);
      ...
      if (this.child == CHILD_POSTPROCESS_INTERPOLATION) {
        ...
        CLIMATIC_SolarForecast = this.Spinner(X_control, Y_control, 1, 0, 0, "Climate-based solar forecast", CLIMATIC_SolarForecast, 0, 1, 1, 1);
        CLIMATIC_WeatherForecast = this.Spinner(X_control, Y_control, 1, 0, 0, "Climate-based temperature forecast", CLIMATIC_WeatherForecast, 0, 2, 1, 1);
      }
      if (this.child == CHILD_POSTPROCESS_DEVELOPED) {
        Develop_Option = this.Spinner(X_control, Y_control, 1, 0, 0, "Develop_Option", Develop_Option, 0, 11, 1, 1);
        Develop_DayHour = this.Spinner(X_control, Y_control, 1, 0, 0, "Develop_DayHour", Develop_DayHour, 0, 3, 1, 1);

        STUDY.TrendJoinHours = this.Spinner(X_control, Y_control, 1, 0, 0, "Trend period hours", STUDY.TrendJoinHours, 1, 24 * 16, 1, 1);
      ...
      if (this.child == CHILD_POSTPROCESS_IMPACTS) {
        CurrentDataSource = this.Spinner(X_control, Y_control, 1, 0, 0, "Impact Source", CurrentDataSource, 0, MAXIMUM_dataID, 1, 1);
        STUDY.ImpactLayer = this.Spinner(X_control, Y_control, 1, 0, 0, "Impact Min/50%/Max", STUDY.ImpactLayer, 0, 8, 1, 1);
      ...
      if (this.child == CHILD_EXPORT_DATA) {
        ...
        User3D.export_FlipZYaxis = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-export.FlipZYaxis", User3D.export_FlipZYaxis, 0, 1, 1, 1);

        User3D.export_PrecisionVertex = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-export.PrecisionVertex", User3D.export_PrecisionVertex, 0, 6, 1, 1);
        User3D.export_PrecisionVtexture = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-export.PrecisionVtexture", User3D.export_PrecisionVtexture, 0, 6, 1, 1);
        User3D.export_PolyToPoly = this.Spinner(X_control, Y_control, 0, 0, 0, "3D-export.PolyToPoly", User3D.export_PolyToPoly, 0, 1, 1, 1);
      ...
      if (this.child == CHILD_EXPORT_MEDIA) {
        ...
        allSolidImpacts.record_IMG = this.Spinner(X_control, Y_control, 0, 0, 0, "Record SolidImpact in JPG", allSolidImpacts.record_IMG, 0, 1, 1, 1);
        allSolidImpacts.record_PDF = this.Spinner(X_control, Y_control, 0, 0, 0, "Record SolidImpact in PDF", allSolidImpacts.record_PDF, 0, 1, 1, 1);

        allSolarImpacts.record_IMG = this.Spinner(X_control, Y_control, 0, 0, 0, "Record Solar Analysis in JPG", allSolarImpacts.record_IMG, 0, 1, 1, 1);
      }
